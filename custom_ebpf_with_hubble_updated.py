...
class SystemHealthAggregator:
    ...
    def get_aggregated_metrics(self, window_duration):
        ...
        try:
            ...
            # cgroup v1 (legacy)
            v1_paths = glob.glob('/sys/fs/cgroup/cpu/kubepods*/*/cpu.stat')
            v1_paths += glob.glob('/sys/fs/cgroup/cpu/kubepods*/*/*/cpu.stat')
            cgroup_paths.extend(v1_paths)
            
            # If no pod cgroups found, use root cgroup
            if not cgroup_paths:
                        ...
                        for line in f:
                            ...
                            if len(parts) == 2:
                                if parts[0] == 'nr_periods':
                                    ...
                                    total_throttled += int(parts[1])
                                elif parts[0] == 'throttled_usec':
                                    # cgroup v2: already in microseconds
                                    total_throttled_usec += int(parts[1])
                                elif parts[0] == 'throttled_time':
                                    # cgroup v1: in nanoseconds
                                    total_throttled_usec += int(parts[1]) / 1000
                except:
                    continue
            ...
            metrics['cpu_nr_periods'] = 0
            metrics['cpu_nr_throttled'] = 0
            metrics['cpu_throttled_time_ms'] = 0.0
            metrics['cpu_throttle_ratio'] = 0.0
        ...
        # Disk queue depth (I/O saturation indicator)
        try:
            # Get queue depth from /sys/block/*/inflight
            queue_depth_total = 0
            block_devices = [d for d in os.listdir('/sys/block') if not d.startswith('loop')]
            
            for device in block_devices:
                inflight_path = f'/sys/block/{device}/inflight'
                if os.path.exists(inflight_path):
                    with open(inflight_path, 'r') as f:
                        # Format: read_inflight write_inflight
                        parts = f.read().strip().split()
                        if len(parts) >= 2:
                            queue_depth_total += int(parts[0]) + int(parts[1])
            
            metrics['disk_queue_depth'] = queue_depth_total
        except:
            metrics['disk_queue_depth'] = 0
        ...
        # TCP backlog overflow stats from /proc/net/netstat
        try:
            overflow_idx = -1
            drop_idx = -1
            with open('/proc/net/netstat', 'r') as f:
                ...
                for i, line in enumerate(lines):
                    if line.startswith('TcpExt:'):
                        ...
                        headers = line.split()
                        try:
                            overflow_idx = headers.index('ListenOverflows')
                            drop_idx = headers.index('ListenDrops')
                        except ValueError:
                            continue
                        ...
                        if i + 1 < len(lines):
                            ...
                            if len(values) > max(overflow_idx, drop_idx):
                                metrics['tcp_listen_overflows'] = int(values[overflow_idx])
                                metrics['tcp_listen_drops'] = int(values[drop_idx])
                                break
            
            if overflow_idx == -1:
                metrics['tcp_listen_overflows'] = 0
                metrics['tcp_listen_drops'] = 0
        except Exception as e:
            metrics['tcp_listen_overflows'] = 0
            metrics['tcp_listen_drops'] = 0
        ...
        metrics['cpu_throttled'] = 1 if metrics.get('cpu_throttle_ratio', 0) > 0.1 else 0  # >10% throttled
        ...
        metrics['disk_saturated'] = 1 if metrics['disk_queue_depth'] > 10 else 0  # >10 pending I/O
...
class MLFeatureAggregator:
    ...
    def _extract_features(self, flow_key, window_id):
        ...
        # ===== NEW: Network Latency Jitter Detection =====
        # Calculate inter-arrival time variance (detects network instability)
        inter_arrival_times = []
        if len(valid_packets) > 1:
            timestamps = [p.timestamp / 1e9 for p in valid_packets]
            inter_arrival_times = [timestamps[i+1] - timestamps[i] for i in range(len(timestamps)-1)]
            if inter_arrival_times:
                import statistics
                features['inter_arrival_mean_ms'] = statistics.mean(inter_arrival_times) * 1000
                features['inter_arrival_std_ms'] = statistics.stdev(inter_arrival_times) * 1000 if len(inter_arrival_times) > 1 else 0
                features['inter_arrival_jitter'] = features['inter_arrival_std_ms'] / features['inter_arrival_mean_ms'] if features['inter_arrival_mean_ms'] > 0 else 0
            else:
                features['inter_arrival_mean_ms'] = 0
                features['inter_arrival_std_ms'] = 0
                features['inter_arrival_jitter'] = 0
        else:
            features['inter_arrival_mean_ms'] = 0
            features['inter_arrival_std_ms'] = 0
            features['inter_arrival_jitter'] = 0
        
        # Calculate tail-latency percentiles from inter-arrival times
        if len(inter_arrival_times) >= 2:
            sorted_times_ms = sorted([t * 1000 for t in inter_arrival_times])
            p50_idx = int(len(sorted_times_ms) * 0.50)
            p95_idx = int(len(sorted_times_ms) * 0.95)
            p99_idx = int(len(sorted_times_ms) * 0.99)
            
            features['latency_p50'] = sorted_times_ms[p50_idx]
            features['latency_p95'] = sorted_times_ms[p95_idx] if p95_idx < len(sorted_times_ms) else sorted_times_ms[-1]
            features['latency_p99'] = sorted_times_ms[p99_idx] if p99_idx < len(sorted_times_ms) else sorted_times_ms[-1]
            features['tail_latency_ratio'] = features['latency_p99'] / features['latency_p50'] if features['latency_p50'] > 0 else 0
        else:
            features['latency_p50'] = 0
            features['latency_p95'] = 0
            features['latency_p99'] = 0
            features['tail_latency_ratio'] = 0
        ...
        if tcp_packets:
            ...
            # ===== NEW: Backpressure Detection =====
            # Track receive window size for backpressure signals
            window_sizes = [p.tcp_window for p in tcp_packets if p.tcp_window > 0]
            if window_sizes:
                features['tcp_window_min'] = min(window_sizes)
                features['tcp_window_avg'] = sum(window_sizes) / len(window_sizes)
                features['tcp_window_drops'] = sum(1 for i in range(len(tcp_packets)-1) 
                                                   if tcp_packets[i+1].tcp_window < tcp_packets[i].tcp_window * 0.5)
            else:
                features['tcp_window_min'] = 0
                features['tcp_window_avg'] = 0
                features['tcp_window_drops'] = 0
            ...
            # Count TCP control packets
            # Use BPF map for SYN count (more accurate for flood detection)
            syn_stats = self.get_syn_flood_stats()
            if src_ip in syn_stats:
                features['syn_count'] = syn_stats[src_ip]
            else:
                # Fallback to counting from captured packets
                features['syn_count'] = sum(1 for p in tcp_packets if (p.tcp_flags & 0x02) and not (p.tcp_flags & 0x10))
        ...
        # ===== NEW: DNS Latency Tracking =====
        features['dns_query_latency_ms'] = 0.0
        if dns_queries and dns_responses:
            # Match queries to responses by timing (simple heuristic)
            latencies = []
            for query in dns_queries:
                for response in dns_responses:
                    if response.timestamp > query.timestamp:
                        latency_ms = (response.timestamp - query.timestamp) / 1e6
                        if latency_ms < 5000:  # Max 5 seconds to be valid
                            latencies.append(latency_ms)
                            break
            if latencies:
                features['dns_query_latency_ms'] = sum(latencies) / len(latencies)
                features['dns_query_latency_p95'] = sorted(latencies)[int(len(latencies) * 0.95)] if len(latencies) > 1 else latencies[0]
        else:
            features['dns_query_latency_p95'] = 0.0
        ...
        features['connection_churn'] = 0.0
        features['abnormal_termination_ratio'] = 0.0
        
        if tcp_packets:
            # Half-open: SYN without SYN-ACK
            syn_only = sum(1 for p in tcp_packets if (p.tcp_flags & 0x02) and not (p.tcp_flags & 0x10))
            features['half_open_connections'] = syn_only
            
            # Connection rate uses syn_count already calculated
            features['connection_rate'] = features['syn_count'] / window_duration if window_duration > 0 else 0
            
            # Connection churn: completed connections per second
            completed = features['fin_count'] + features['rst_count']
            features['connection_churn'] = completed / window_duration if window_duration > 0 else 0
            
            # Abnormal terminations (RST indicates abrupt close)
            features['abnormal_termination_ratio'] = features['rst_count'] / completed if completed > 0 else 0
    ...
    def get_features_df(self):
        ...
        # Optimized feature set: 94 total features (removed redundant new_connections)
        # 49 network + 17 L7 + 21 system + 7 metadata = 94 features
        column_order = [
            # Metadata (8)
            'timestamp', 'flow_id', 'src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol', 'l7_protocol',
            # Network features (49) - core + fault detection (removed new_connections redundancy)
            ...
            # Fault detection features (13) - removed new_connections
            'inter_arrival_mean_ms', 'inter_arrival_std_ms', 'inter_arrival_jitter',  # Latency jitter
            'latency_p50', 'latency_p95', 'latency_p99', 'tail_latency_ratio',  # Tail-latency
            'tcp_window_min', 'tcp_window_avg', 'tcp_window_drops',  # Backpressure
            'connection_rate', 'connection_churn', 'abnormal_termination_ratio',  # Connection churn
            # System metrics (21) - expanded with K8s-specific metrics
            'cpu_usage_percent', 'process_cpu_top3',
            'cpu_nr_periods', 'cpu_nr_throttled', 'cpu_throttled_time_ms', 'cpu_throttle_ratio',  # K8s CPU throttling
            'memory_usage_percent', 'memory_available_mb', 'swap_usage_percent', 'oom_kill_count',
            'disk_read_mb', 'disk_write_mb', 'disk_avg_latency_us', 'disk_queue_depth',  # Queue depth
            'tcp_listen_overflows', 'tcp_listen_drops',  # TCP backlog
            'process_exits', 
            'high_io_latency', 'cpu_contention', 'cpu_throttled', 'memory_critical', 'swap_active', 'disk_saturated',  # Binary flags
            # Hubble L7 features (17)
            'http_status_code', 'http_method', 'http_latency_ms',
            'grpc_status_code', 'grpc_method',
            'dns_response_code', 'dns_num_answers', 'dns_query_latency_ms', 'dns_query_latency_p95',
